...
Optimize herbal essential-oil extraction: method, parameters, yield & aroma profile, preserving volatiles per ISO standards.

**Gate:** Method & parameters set; GC-MS profile; yield & aroma preservation; ISO standard.
...
**Gate:** Conclusion is exactly one of: Optimal Extraction / Conditional (thermal) / Yield/Aroma Loss / Inconclusive; disclosure appears before the conclusion.
...
| `sub-core-analysis` | Step 3 — Optimize herbal essential-oil extraction: method, parameters, yield &  |
...
- **WebSearch** / **WebFetch** — Essential Oil Extraction & Aromatic Chemistry sources
...
```
# Herbal Essential Oil Extraction Process Optimization — Report
**Date:** YYYY-MM-DD | **Analyst:** herbal-essential-oil-extraction v1.0 | **Language:** Vietnamese/English | **Domain:** Essential Oil Extraction & Aromatic Chemistry
...
This harness is implemented as a deterministic, testable Python runtime:
- Router: src/herbal_oil/core/router.py (ChainOfThoughtRouter: intent -> ordered plan, comparison repeat-steps, educational skip).
- Runner: src/herbal_oil/core/runner.py (orchestration + quality gates U1-U6/G1-G4 with auto-fix + 2-retry budget + Markdown report).
- Registry: src/herbal_oil/core/registry.py (agents/tools/hooks resolved by name).
- Wiring: src/herbal_oil/factory.py -> uild_runner().

See SKILL.md for the registry contract and ssets/schemas/*.schema.json for I/O schemas.
